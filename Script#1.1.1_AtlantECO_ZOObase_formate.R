...
files.gbif <- dir()[grep("_GBIF_",dir())][1:15] # Because 16:17 is ZooBase
files.gbif
...
res <- mclapply(files.gbif, function(f) {
                ...
                if(f == "Hydrozoa_GBIF_23_04_18.Rdata") {
                    ...
                    d$decimallongitude <- as.numeric(as.character(d$decimallongitude))
                    d <- d %>% drop_na(decimallatitude)
                    d <- d %>% drop_na(decimallongitude)

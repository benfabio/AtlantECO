...
##### ATLANTECO SCRIPT 1.4.5 ----------------------------------------------------------------------------------------------------------------------------
##### 12/11/2021: R Script to examine and format of COPEPOD-NOAA Chaetognatha dataset and reformat it following the ATLANTECO WP2 template © Fabio Benedetti, ETH Zürich, IBP, UP Group.
...
data <- read.csv("COPEPOD_Chaetognatha_4320000_12_11_21.csv", sep = ";", dec = ",", h = T)
dim(data) # 24'945
str(data)
...
data2 <- data.frame(ProjectID = "AtlantECO_H2020_GA#862923", ProjectWP = "WP2", DataSilo = "Trad_microscopy", ContactName = "Fabio_Benedetti;Meike_Vogt",
                ...
                orig_occurrenceID = data$RECORD.ID, obisID = "Not_applicable", DatasetKey = "4320000",
                ...
                SourceArchive = "https://www.st.nmfs.noaa.gov/copepod/atlas/html/taxatlas_4320000.html",
...
# Aidanosagitta crassa naikaiensis                     no match                                 
# Sagittidea                                           no match                                 
# Sagitta serratodentata pseudoserratodentata          no match                                 
# Chaetognatha parts/fragments                         no match                                 
# Chaetognatha unidentified                            no match                                 
# Sagitta serratodentata pacifica                      no match                                 
# Sagitta bedoti minor                                 no match  

# To correct manually
data2[data2$ScientificName == "Aidanosagitta crassa naikaiensis","ScientificName"] <- "Aidanosagitta crassa"
data2[data2$ScientificName == "Sagittidea","ScientificName"] <- "Sagittidae"
data2[data2$ScientificName == "Sagitta serratodentata pseudoserratodentata","ScientificName"] <- "Serratosagitta serratodentata"
data2[data2$ScientificName == "Chaetognatha parts/fragments","ScientificName"] <- "Chaetognatha"
data2[data2$ScientificName == "Chaetognatha unidentified","ScientificName"] <- "Chaetognatha"
data2[data2$ScientificName == "Sagitta serratodentata pacifica","ScientificName"] <- "Serratosagitta pacifica"
data2[data2$ScientificName == "Sagitta bedoti minor","ScientificName"] <- "Zonosagitta bedoti"
...
save(ddf, file = "COPEPOD-NOAA_Chaetognatha_4320000_reformatted+WoRMScheck_15_11_21.Rdata")
write.table(ddf, file = "COPEPOD-NOAA_Chaetognatha_4320000_reformatted+WoRMScheck_15_11_21.txt", sep = "\t")

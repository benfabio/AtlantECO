...
##### ATLANTECO SCRIPT 1.4.3 ----------------------------------------------------------------------------------------------------------------------------
##### 11/11/2021: R Script to examine and format of COPEPOD-NOAA Euphausiacea dataset and reformat it following the ATLANTECO WP2 template © Fabio Benedetti, ETH Zürich, IBP, UP Group.
...
data <- read.csv("COPEPOD_Euphausiacea_4218000_09_11_21.csv", sep = ";", dec = ",", h = T)
dim(data) # 37'450 records
str(data)
...
data[data$TIMEgmt > 24 & !is.na(data$TIMEgmt),"TIMEgmt"] <- NA
...
data2 <- data.frame(ProjectID = "AtlantECO_H2020_GA#862923", ProjectWP = "WP2", DataSilo = "Trad_microscopy", ContactName = "Fabio_Benedetti;Meike_Vogt",
                ...
                orig_occurrenceID = data$RECORD.ID, obisID = "Not_applicable", DatasetKey = "4218000",
                ...
                SourceArchive = "https://www.st.nmfs.noaa.gov/copepod/atlas/html/taxatlas_4218000.html",
...
# 11 levels to correct
str(data2$ScientificName)
data2[data2$ScientificName == "Tessarabrachion oculatus","ScientificName"] <- "Tessarabrachion oculatum"
data2[data2$ScientificName == "Euphausiacea unidentified","ScientificName"] <- "Euphausiacea"
data2[data2$ScientificName == "Euphausiacea TOTAL (total count)","ScientificName"] <- "Euphausiacea"
...
data2[data2$ScientificName == "Thysanoessanifera","ScientificName"] <- "Thysanoessa spinifera" # Ooopsie
...
### Check lifeforms
unique(ddf$LifeForm) # Ok, easy, just need to remove brackets
# gsub("\\[|\\]", "", n)

# And check some random rows to make sure the formatting went all across OrigScientificName, ScientificName and LifeForm
# colnames(ddf)
ddf[1000:1020,c("OrigScientificName","ScientificName","LifeForm","TaxonRank")]
# Looks ganz OK :-)
...
save(ddf, file = "COPEPOD-NOAA_Euphausiacea_4218000_reformatted+WoRMScheck_11_11_21.Rdata")
write.table(ddf, file = "COPEPOD-NOAA_Euphausiacea_4218000_reformatted+WoRMScheck_11_11_21.txt", sep = "\t")
